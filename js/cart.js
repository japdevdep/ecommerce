...
function showArticles(articles){
    let articlesWrapperHTML = document.getElementById("articlesWrapper");

    let htmlContentToAppend = "";

    for(let i = 0; i < articles.length; i++){
        let imageSrc = articles[i];

        htmlContentToAppend += `
        <tr>
            <td><img src='`+ articles[i].src + `' width="150px"></td>
            <td>`+ articles[i].name + `</td>
            <td>`+ articles[i].count + `</td>
            <td>`+ articles[i].unitCost + +`</td>
            <td>`+ articles[i].currency+`</td>
        </tr>
        `
